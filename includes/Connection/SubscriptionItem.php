...
namespace WPGraphQL\WooCommerce\ProductSubscriptions\Connection\SubscriptionItems;
...
use const WPGraphQL\WooCommerce\ProductSubscriptions\TYPE_SUBSCRIPTION_PRODUCT;
...
add_action( 'graphql_register_types', function () {
		...
		return [
			'quantityMin'                        => [
				...
				'resolve'     => function ( $source ) {
					/* @var $bundledItem WCS_ATT_Product */
					$bundledItem = $source['bundledItem'];
					
					if ( empty( is_a( $bundledItem, \WCS_ATT_Product::class ) ) ) {
						return null;
					}
	...
	register_graphql_connection( [
		'fromType'      => TYPE_SUBSCRIPTION_PRODUCT,
		'toType'        => 'Product',
...
/**
 * Inject the WCS_ATT_Product Class into the resolver so that fields can access
 * edge data
 */
add_filter( 'graphql_connection_edges', function ( $edges, AbstractConnectionResolver $resolver ) {
	...
	foreach ( $edges as $key => $edge ) {
		...
		$raw_bundled_data_item = array_filter( $bundled_data_items,
			function ( WCS_ATT_Product $item ) use ( $edge ) {
